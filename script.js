...
function init() {
    ...
    app.innerHTML = `
        ...
        <input type="number" id="waterInput" placeholder="Add water (ml)" ${isGoalReached ? 'disabled' : ''}>
        
        <button id="addBtn" onclick="addWater()" 
            style="${isGoalReached ? 'background:#555; cursor:not-allowed; opacity:0.6;' : ''}" 
            ${isGoalReached ? 'disabled' : ''}>
            ${isGoalReached ? 'Daily Goal Met!' : 'Add Drink'}
        </button>
